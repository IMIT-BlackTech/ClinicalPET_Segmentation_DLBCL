"""
bone segmentation using threshold and morphology method
by JZ
...
def ct_bed_removal_bone(ct:Union[str, np.ndarray], suv:Union[str, np.ndarray], organ_seg:Union[str, np.ndarray]):
    ...
    if isinstance(ct, str):
        assert isfile(ct), 'if isinstance(ct, str), isfile(ct) must be true!'
        ct,origin, spacing, direction = nii2numpy(ct)


    if isinstance(suv, str):
        assert isfile(suv), 'if isinstance(suv, str), isfile(suv) must be true!'
        suv,origin, spacing, direction = nii2numpy(suv)
