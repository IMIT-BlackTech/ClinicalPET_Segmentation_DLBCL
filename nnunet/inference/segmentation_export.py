...
from nnunet.inference.bone_seg_utils import ct_bed_removal_bone
...
def save_organ_segmentation_nifti_from_softmax(ct_file:Union[str, np.ndarray], pet_img_suv:Union[str, np.ndarray], segmentation_softmax: Union[str, np.ndarray], out_fname: str,
                                         properties_dict: dict, order: int = 1,
                                         region_class_order: Tuple[Tuple[int]] = None,
                                         seg_postprogess_fn: callable = None, seg_postprocess_args: tuple = None,
                                         resampled_npz_fname: str = None,
    ...
    whole_bone, lymph_mask_raw = ct_bed_removal_bone(ct_file, pet_img_suv, seg_old_size_postprocessed)
    ...
    # save bone mask
    seg_resized_itk = sitk.GetImageFromArray(whole_bone.astype(np.uint8))
    seg_resized_itk.SetSpacing(properties_dict['itk_spacing'])
    seg_resized_itk.SetOrigin(properties_dict['itk_origin'])
    seg_resized_itk.SetDirection(properties_dict['itk_direction'])
    sitk.WriteImage(seg_resized_itk, 
                out_fname.replace(out_fname.split('/')[-1],
                out_fname.split('/')[-1].replace('.nii.gz', '_bone.nii.gz')))

    # lymph
    seg_resized_itk = sitk.GetImageFromArray(lymph_mask_raw.astype(np.uint8))
    seg_resized_itk.SetSpacing(properties_dict['itk_spacing'])
    seg_resized_itk.SetOrigin(properties_dict['itk_origin'])
    seg_resized_itk.SetDirection(properties_dict['itk_direction'])
    sitk.WriteImage(seg_resized_itk, 
                out_fname.replace(out_fname.split('/')[-1],
                out_fname.split('/')[-1].replace('.nii.gz', '_lymph.nii.gz')))

    ## save pet suv image
    seg_resized_itk = sitk.GetImageFromArray(pet_img_suv.astype(np.float32))
    seg_resized_itk.SetSpacing(properties_dict['itk_spacing'])
    seg_resized_itk.SetOrigin(properties_dict['itk_origin'])
    seg_resized_itk.SetDirection(properties_dict['itk_direction'])
    sitk.WriteImage(seg_resized_itk, 
                out_fname.replace(out_fname.split('/')[-1],
                out_fname.split('/')[-1].replace('.nii.gz', '_suv.nii.gz')))
